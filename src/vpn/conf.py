...
AND do this for every peer by running ./add-routes.sh if it exists

          ip -4 route add 10.11.200.100/32 dev wg || true

AND also the pings.sh script
...
"""
    ...
    if 'external_ip' not in interface or not interface['external_ip']:
        ...
        for peer in peers:
            pings += f"ping -r -I wg -n -i 15 -c 4 {peer['vpn_ip']} &\n"
        if len(peers) > 0:
            open('pings.sh', 'w').write(pings)
            os.chmod('pings.sh', 0o755)

    if len(peers) > 0:
        add_routes = '#!/usr/bin/env bash\n\nset -v\n\n'
        for peer in peers:
            add_routes += f"ip -4 route add {peer['vpn_ip']}/32 dev wg || true\n"
        open('add-routes.sh', 'w').write(add_routes)
        os.chmod('add-routes.sh', 0o755)
