...
def ensure_cloud_filesystem_container_is_running(image):
    ...
    # The config is in /cocalc/conf.
    cmd = f"mkdir -p  /data/.cloud-filesystem; chown user:user /data/.cloud-filesystem; exec docker run -d --init --network host --name cloud-filesystem --privileged -v /data/.cloud-filesystem:/var/cloud-filesystem -v /cocalc/conf:/cocalc/conf --mount type=bind,source=/home/user,target=/home/user,bind-propagation=rshared {image} & "
    print(cmd)
    os.system(cmd)
